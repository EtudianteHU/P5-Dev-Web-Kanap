...
btnSelector.addEventListener('click', (event) => {
    ...
    if (quantityNumber <= 0 || quantityNumber > 100) { // si la qté est inférieure à 0 ou supérieure à 100, on affiche un mesage d'erreur
    ...
    alert('Le produit a été commandé')
})



const btnSelector = document.querySelector('#addtocard')
btnSelector addEventListener('click', (event) => {
    const selectedColor = document.querySelector('#colors').value
    const quantityString = document.querySelector('#quantity').value
    ...
    if (quantityNumber <= 0 || quantityNumber = 100 {
        alerte(Veuillez saisir une quantité  entre 1 et 100)
        return
    }
        )
