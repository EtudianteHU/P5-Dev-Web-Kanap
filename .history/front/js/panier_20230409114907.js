...
inputsQuantity.forEach((input) => {
  input.addEventListener('change', (event) => {
    const totalQuantityPanier = panierObject.find(product)
    ...
    const totalPricePanier = panierObject.find(product)
  ...
  deleteLinks.forEach((link) => {
    link.addEventListener('click', (event) => {
      const productTotal = panierObject.find(product)
      console.log(productTotal)
      productTotal.innerHTML =

      const priceTotal = panierObject.find(product)
