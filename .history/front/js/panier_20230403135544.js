...
panierObject.forEach(async (panier, index) => {
  const product = await getProduct(panier._id)
  console.log('contenu du product', product)
  console.log('contenu de panier', panier)

  contenuItem.innerHTML += ` <article class="cart__item" data-id="${panier._id}" data-color="${panier.color}">
...
let totalQuantity = 0
let totalPrice = 0
panierObject.forEach((function (product) {
  totalQuantity = totalQuantity + product.quantity; // meme chose:  totalQuantity += product.quantity;  
